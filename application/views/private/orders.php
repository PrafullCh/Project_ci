...
<body>
	...
	<table  width="850" cellpadding="10">
		...
		<?php foreach($orders as $i):?>
			<form action="<?= base_url('Loadbooks/delete_order')?>" method="post">
		<tr>
						<td><?= $orders[$j]->BOOK_NAME?></td>
			<td><?= $orders[$j]->B_QTY?></td>
			<td><?= $orders[$j]->T_PRICE?></td>
			<td><?= $orders[$j]->exp_del?></td>
			<td><?= $orders[$j]->status?></td>
			<td><?= $orders[$j]->ORDER_ID?></td>
			<input type="hidden" name="ORDER_ID" value="<?= $orders[$j]->ORDER_ID?>">
